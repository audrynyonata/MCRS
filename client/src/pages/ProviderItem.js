  ...
  render() {
    ...
    return (
      <Container fluid className="pt-3">
        {this.props.provider ? <div>
          <Title xs={12} md={{ span: 8, offset: 2 }}>{this.props.provider.name}</Title>
          <Row>
            <Col md={{ span: 6, offset: 2 }}>
              ...
              <ol>
                {this.props.provider.urls.map((e, idx) =>
                  <li key={idx}>{e.name} - <a href={`/${e.url}`}>{e.url}</a></li>)}
              </ol>
            ...
            <Col xs={4}>
              ...
              <ul>
                {this.props.provider.contacts.map((e, idx) =>
                  <li key={idx}>{e.name} {e.role} {e.description} {e.email} {e.phone} {e.address}</li>
                )}
              </ul>
            </Col>
          </Row></div> : ""}
...
const mapStateToProps = (state, ownProps) => {
  console.log("ownProps", ownProps); return ({
    ...state.providerReducer,
    provider: state.providerReducer.providers.find(e =>
      e.id.toLowerCase() === ownProps.match.params.id.toLowerCase()
    )
  })
}
